...
use crate::fresh::FreshLinearInstance;
use crate::frontend::{QuasarFrontendOutput, QuasarFrontendProver};
...
pub enum QuasarFoldError {
    ...
    FiatShamir(#[from] crate::fiat_shamir::errors::FiatShamirError),
    ...
    [W; DIGEST_ELEMS]: serde::Serialize + for<'de> serde::Deserialize<'de>,
{
    ...
    fn init(config: &Self::Config) -> Self::Accumulator {
        ...
        Accumulator::new(
            AccumulatorInstance {
                // SAFETY: `commitment_root` is a dummy placeholder for the initial accumulator.
                // Quasar's first fold creates a real accumulator from scratch, replacing this
                // value before it is ever read as a real commitment. The zeroed digest
                // ([0u8; DIGEST_ELEMS]) is a valid bit pattern for any `Hash<F, W, DIGEST_ELEMS>`
                // since it is a plain array of `Copy` + `Default` elements.
