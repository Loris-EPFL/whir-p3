...
fn rebuild_acc(r: &WarpFoldResult<F>) -> WarpAccumulator<F, F, F, DIGEST> {
    let ec = whir_p3::warp::fold::evaluate_mle_lsb(
        &r.witness.codeword,
...
fn run_non_recursive_fold(
    ...
    for step in 0..num_steps {
        ...
        let result = warp_fold_prove_rs_committed(
            ...
            |cw, ff| {
                let (root, _) =
                    merkle_commit_codeword::<F, F, <F as Field>::Packing, <F as Field>::Packing, MyHash, MyCompress, DIGEST>(
                        cw,
                        ff,
                        mhc.clone(),
                        mcc.clone(),
...
fn run_regular_recursive_union(
    ...
    for s in 0..num_steps {
        ...
        let (union_root, _union_tree) = merkle_commit_codeword::<F, F, <F as Field>::Packing, <F as Field>::Packing, MyHash, MyCompress, DIGEST>(
...
fn run_cp_snark_recursive(
    ...
    let (fresh_root0, fresh_tree0) = merkle_commit_codeword::<F, F, <F as Field>::Packing, <F as Field>::Packing, MyHash, MyCompress, DIGEST>(
    ...
    let (acc_actual_root0, acc_tree0) = merkle_commit_codeword::<F, F, <F as Field>::Packing, <F as Field>::Packing, MyHash, MyCompress, DIGEST>(
        &zero_acc.witness.codeword, rs_config.folding_factor, mh.clone(), mc.clone(),
    );
    {
        use whir_p3::warp::fold::open_shift_queries_from_trees;
    ...
    for s in 0..num_steps {
        ...
        let (fresh_root, fresh_tree) = merkle_commit_codeword::<F, F, <F as Field>::Packing, <F as Field>::Packing, MyHash, MyCompress, DIGEST>(
        ...
        // Open shift query auth paths from pre-built trees (no redundant rebuild)
        let (_, acc_tree) = merkle_commit_codeword::<F, F, <F as Field>::Packing, <F as Field>::Packing, MyHash, MyCompress, DIGEST>(
...
fn run_union_cp_snark(
    ...
    for step in 0..num_steps {
        ...
        let union_cw = build_union_codeword(&all_cw);
        let union_ff = whir_p3::warp::encoding::union_folding_factor(
            rs_config.folding_factor, l,
        );
        let union_ev = EvaluationsList::new(union_cw);
        let (union_root, union_tree) = merkle_commit_codeword::<F, F, <F as Field>::Packing, <F as Field>::Packing, MyHash, MyCompress, DIGEST>(
            &union_ev, union_ff, mh.clone(), mc.clone(),
        );
        ...
        {
            use whir_p3::warp::fold::open_shift_queries_from_union_tree;
