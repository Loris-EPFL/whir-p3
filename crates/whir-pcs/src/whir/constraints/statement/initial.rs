...
mod tests {
    ...
    fn make_test_poly() -> EvaluationsList<F> {
    ...
    fn make_large_poly() -> EvaluationsList<F> {
    ...
    fn test_evaluate_classic() {
        ...
        let point = MultilinearPoint::new(vec![EF::ZERO, EF::ONE]);
    ...
    fn test_evaluate_extension_point() {
        ...
        let expected = poly.evaluate_hypercube_base(&point);
    ...
    fn test_multiple_evaluations() {
        ...
        let point1 = MultilinearPoint::new(vec![EF::ZERO, EF::ZERO]);
        let point2 = MultilinearPoint::new(vec![EF::ONE, EF::ONE]);
        let point3 = MultilinearPoint::new(vec![EF::from_u64(5), EF::from_u64(7)]);
        ...
        assert_eq!(eval3, poly.evaluate_hypercube_base(&point3));
    ...
    fn test_evaluate_wrong_variable_count() {
        ...
        let wrong_point = MultilinearPoint::new(vec![EF::ONE, EF::ZERO, EF::ONE]);
    ...
    fn test_normalize_classic() {
        ...
        let point1 = MultilinearPoint::new(vec![EF::ZERO, EF::ONE]);
        let point2 = MultilinearPoint::new(vec![EF::ONE, EF::ZERO]);
        let eval1 = statement.evaluate(&point1);
        let eval2 = statement.evaluate(&point2);
    ...
    fn test_normalize_svo() {
        ...
        let point = MultilinearPoint::new((0..16).map(|i| EF::from_u64(i as u64)).collect());
        let eval = statement.evaluate(&point);
    ...
    fn test_inner_evaluate_classic() {
        ...
        let point = MultilinearPoint::new(vec![EF::ONE, EF::ZERO]);
    ...
    fn test_svo_creates_split_eq() {
        ...
        let point = MultilinearPoint::new((0..16).map(|_| EF::from_u64(1)).collect());
        let eval = inner.evaluate(&point, &poly);
