...
fn main() {
    ...
    writeln!(
        metrics,
        "shape,log_m,num_cons,num_vars,nnz_per_row,nnz_per_matrix_global,total_nnz_global,prover_ms,verifier_ms,opening_unbatched_us,opening_batched_us"
    )
    .unwrap();
    ...
    writeln!(
        opening,
        "shape,log_m,num_cons,num_vars,nnz_per_row,nnz_per_matrix_global,queries,unbatched_us,batched_us,unbatched_field_ops,batched_field_ops,unbatched_serialized_fields,batched_serialized_fields,unbatched_full_proof_bytes,batched_full_proof_bytes"
    )
    .unwrap();
    ...
    writeln!(
        verifier_queries,
        "shape,log_m,num_cons,num_vars,nnz_per_row,nnz_per_matrix_global,queries,verifier_unbatched_us,verifier_batched_us"
    )
    .unwrap();
    ...
    for shape in ShapeKind::all() {
        for &log_m in &log_ms {
            ...
            for nnz_per_row in nnz_per_row_values {
                ...
                let material = build_scenario(&scenario);
                let nnz_per_matrix_global = nnz_per_row * num_cons;
                let total_nnz_global = 3 * nnz_per_matrix_global;

                let (prover_ms, verifier_ms) = bench_prover_verifier(&material, repeats);
                ...
                for queries in query_grid {
                    ...
                    let batched_field_ops = queries * nnz_per_matrix_global + 2 * queries;
...
fn bench_prover_verifier(material: &ScenarioMaterial, repeats: usize) -> (f64, f64) {
    ...
    for i in 0..repeats {
        ...
        let perm = Perm::new_from_rng_128(&mut rand::rngs::SmallRng::seed_from_u64(seed));
...
fn bench_opening_pair(material: &ScenarioMaterial, queries: usize, repeats: usize) -> (f64, f64) {
    ...
    let points: Vec<(Vec<F>, Vec<F>)> = (0..queries)
        .map(|q| {
            (
                deterministic_point(material.shape.num_poly_vars_x(), q as u64 + 11),
                deterministic_point(num_vars_y, q as u64 + 97),
            )
        })
        .collect();
    ...
    let sanity = points
        .iter()
        .map(|(rx, ry)| {
            let (a_eval, b_eval, c_eval) = eval_unbatched(
                &material.a_entries,
                &material.b_entries,
                &material.c_entries,
                rx,
            ...
            let batched_eval = eval_batched_aligned(
                &material.a_entries,
                &material.b_entries,
                &material.c_entries,
                rx,
    ...
    for _ in 0..repeats {
        ...
        for (rx, ry) in &points {
            let batch = eval_batched_aligned(
                &material.a_entries,
                &material.b_entries,
                &material.c_entries,
                rx,
...
fn bench_verifier_queries(
    ...
    let points: Vec<(Vec<F>, Vec<F>)> = (0..queries)
        .map(|q| {
            (
                deterministic_point(material.shape.num_poly_vars_x(), q as u64 + 211),
                deterministic_point(num_vars_y, q as u64 + 307),
            )
        })
        .collect();
    ...
    for _ in 0..repeats {
        ...
        for ((rx, ry), (_, _, _, c_batch)) in points.iter().zip(claims.iter()) {
            let batch_eval = eval_batched_aligned(
                &material.a_entries,
                &material.b_entries,
                &material.c_entries,
                rx,
...
fn eval_sparse(entries: &[SparseMatEntry<F>], rx: &[F], ry: &[F]) -> F {
    let mut acc = F::ZERO;
    for e in entries {
...
fn build_scenario(s: &Scenario) -> ScenarioMaterial {
    ...
    let a_entries = materialize_entries(&positions, 3);
    let b_entries = materialize_entries(&positions, 11);
    let c_entries = materialize_entries(&positions, 29);
...
fn materialize_entries(positions: &[(usize, usize)], salt: u64) -> Vec<SparseMatEntry<F>> {
    positions
        .iter()
        .enumerate()
        .map(|(k, (row, col))| {
...
fn deterministic_point(dim: usize, seed: u64) -> Vec<F> {
    (0..dim)
        .map(|i| {
            let v = (seed
